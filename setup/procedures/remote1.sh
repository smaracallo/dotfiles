...
chown -R remoteuser:remoteuser /etc/ssh/remoteuser

chmod 755 /etc/ssh/remoteuser

chmod 644 /etc/ssh/remoteuser/authorized_keys

sed -i -e '/^#AuthorizedKeysFile/s/^.*$/AuthorizedKeysFile \/etc\/ssh\/remoteuser\/authorized_keys/' /etc/ssh/sshd_config
...
sh -c 'echo "AllowUsers remoteuser" >> /etc/ssh/sshd_config'
...
cat /home/remoteuser/.credentials | chpasswd

rm /home/remoteuser/.credentials
